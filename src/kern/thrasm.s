...
# Inputs: struct thread stack anchor pointer, uintptr_t user stack pointer, and uintptr_t user pc
# Outputs: none
# Purpose: disables and later reenables interrupts, sets sscratch to the thread stack anchor, sets spp and spie bit in sstatus, writes upc to sepc
# changes the sp to usp and writes trap entry from u mode to stvec
_thread_finish_jump:
        ...
        csrc sstatus, 0x2 # disables interrupts  using bit mask 0b10 which sets IE bits

        csrw sscratch, a0 # sets sscratch to trap anchor 
        li t1, 0x100 # loads bit mask 0b1000000 which clears bit 8 corresponding to SPP
        csrc sstatus, t1 # clears spp bit
        csrsi sstatus, 0x8 # sets spie bit      using bit mask 0b1000 which sets SPIE bit
        csrw sepc, a2 # sets sepc to upc
        mv sp, a1 # sets sp to usp
        la t0, _trap_entry_from_umode # loads address
        csrw stvec, t0 # sets stvec to trap entry from umode 

        csrs sstatus, 0x2 # reenables interrupt         using bit mask 0b10 which sets IE bit
...
# Inputs: *child (struct thread), *parent_tfr (struct trap_frame)
# Outputs: none
# Purpose: begins by saving the currently running thread, switches to the new child process thread and back to the u mode 
# interrupt handler. it then restores the saved trap frame which is actually the duplicated parent trap frame. 
_thread_finish_fork:
