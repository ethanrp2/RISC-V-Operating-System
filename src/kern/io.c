...
// static void iolit_close(struct io_intf * io);
    ...
    struct io_lit * lit, void * buf, size_t size)
{
    ...
    static const struct io_ops ops = {      // set the operations for an io_lit
        .close = NULL,
        .read = iolit_read,
        .write = iolit_write,
        .ctl = iolit_ioctl
    ...
    lit->buf = b;               // initialize the members based on the parameters
    lit->size = size;
    lit->io_intf.ops = &ops;
    lit->pos = pos;
    lit->io_intf.refcnt = 1; //ref count


    return &lit->io_intf;               // return a pointer to the interface
}

long iolit_read(struct io_intf * io, void * buf, size_t len){
    struct io_lit * const dev = (void*)io - offsetof(struct io_lit, io_intf);       // retrieve the original io_lit given the io_intf
    ...
    if (dev->pos + len > dev->size){                // clip the length of the read if the size is not large enough
        // console_printf("%d\n", len);
        len = dev->size - dev->pos;
        
    }

    // console_printf("len: %d\n", len);

    memcpy(b, p, len);              // use memcpy to copy data from buffer to buffer

    dev->pos += len;                // update position
...
Inputs: io, cmd, arg
Outputs: int status
Purpose: Returns different information about the io_lit given the command, position/size. Allows caller to control member variables.
*/
static long iolit_write(struct io_intf * io, const void * buf, size_t len){
    struct io_lit * const dev = (void*)io - offsetof(struct io_lit, io_intf);   // retrieve the original io_lit given the io_intf

    char * p = dev->buf + dev->pos;             // get location using position and buffer

    if (dev->pos + len > dev->size){            // clip length if necessary
        len = dev->size - dev->pos;
    }

    memcpy(p, buf, len);                        // use memcpy to copy data from buffer to buffer
    dev->pos += len;                            // update position
...
int iolit_ioctl(struct io_intf * io, int cmd, void * arg){
    ...
    switch (cmd){                                   // return various info based on the given cmd
        case IOCTL_GETLEN:
            *(size_t *)arg = dev->size;
            return 0;
        case IOCTL_GETPOS:
